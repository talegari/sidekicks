...
#'   involving `parallel::mclapply` call. Place the 'memusg.py' file in
#'   '/usr/local/bin' directory and make it executable. The function is meant to
#'   measure peak memory utility in the order of GBs with long running parallel
#'   processes. For smaller processes, the overhead itself will distort the real
#'   usage. For smaller benchmarks, use `benck::mark` or `peakRAM::peakRAM`. For
#'   more details, its better to modify the python program.
